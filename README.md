...
Реализован полный цикл работы с сообщениями в Kafka на примере агентства по продаже авиабилетов. Producer генерирует бронирования (авиакомпания, рейс, аэропорты, пассажир, билет, статус, стоимость) и отправляет их в топик. Consumer читает сообщения из топика, валидирует их и выводит результат в консоль.

Первое сообщение намеренно генерируется с невалидными данными для демонстрации работы валидации.

## Стек технологий

**Apache Kafka 4.0.0** (KRaft mode, без ZooKeeper) — брокер сообщений
